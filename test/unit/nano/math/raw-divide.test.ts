...
describe("rawDivide", () => {
  ...
  test("returns success result in non-throwing mode", () => {
    const result = rawDivide({ raw: "20", divisor: "4", throwOnError: false });
    assert(result.success);
    expect(result.data).toBe("5");
  });

  test("throws when divisor is zero", () => {
    expect(() => rawDivide({ raw: "10", divisor: "0", throwOnError: true })).toThrow(
      "Division by zero is not allowed."
    );
  });

  test("throws when division leaves a remainder", () => {
    expect(() => rawDivide({ raw: "10", divisor: "3", throwOnError: true })).toThrow(
  ...
  test("rejects invalid inputs", () => {
    expect(() => rawDivide({ raw: "", divisor: "1", throwOnError: true })).toThrow("Invalid raw value.");
    expect(() => rawDivide({ raw: "1", divisor: "abc", throwOnError: true })).toThrow("Invalid divisor value.");

    const result = rawDivide({ raw: "abc", divisor: "1", throwOnError: false });
