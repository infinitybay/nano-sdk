...
describe("ReceivableResponse schema", () => {
  test("parses receivable response with empty blocks map", () => {
    const schema = ReceivableResponse({ min_version: false, sorting: false, source: false, threshold: false });
  ...
  test("parses receivable response with min_version, source and threshold", () => {
    const schema = ReceivableResponse({ min_version: true, sorting: false, source: true, threshold: true });
    const result = schema.safeParse({
      blocks: {
  ...
  test("parses receivable response with min_version", () => {
    const schema = ReceivableResponse({ min_version: true, sorting: false, source: false, threshold: false });
    const result = schema.safeParse({
      blocks: {
        [TestData.Valid.Hash1()]: {
          amount: TestData.Valid.RawAmount1(),
  ...
  test("parses receivable response with source", () => {
    const schema = ReceivableResponse({ min_version: false, sorting: false, source: true, threshold: false });
    const result = schema.safeParse({
      blocks: {
        [TestData.Valid.Hash1()]: {
          amount: TestData.Valid.RawAmount1(),
  ...
  test("parses receivable response with threshold", () => {
    const schema = ReceivableResponse({ min_version: false, sorting: false, source: false, threshold: true });
    const result = schema.safeParse({
      blocks: {
  ...
  test("parses sorted receivable blocks as amounts without source or threshold flags", () => {
    const schema = ReceivableResponse({ min_version: false, sorting: true, source: false, threshold: false });
    const result = schema.safeParse({
      blocks: {
        [TestData.Valid.Hash1()]: TestData.Valid.RawAmount1(),
      },
    ...
    assert(result.success);
    assert(result.data.blocks);
    expect(result.data.blocks[TestData.Valid.Hash1()]).toBe(TestData.Valid.RawAmount1());
  });

  test("rejects hash arrays for sorted receivable blocks", () => {
    const schema = ReceivableResponse({ min_version: false, sorting: true, source: false, threshold: false });
    const result = schema.safeParse({
