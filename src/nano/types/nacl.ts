...
export class Nacl {
  static vn(x: Uint8Array, xi: number, y: Uint8Array, yi: number, n: number): number {
    let d = 0;
    for (let i = 0; i < n; i++) d |= x[xi + i] ^ y[yi + i];
    return (1 & ((d - 1) >>> 8)) - 1;
  }

  static cryptoVerify32(x: Uint8Array, xi: number, y: Uint8Array, yi: number): number {
    return this.vn(x, xi, y, yi, 32);
  }

  static set25519(r: Float64Array, a: Float64Array): void {
    let i;
    for (i = 0; i < 16; i++) r[i] = a[i] | 0;
  }

  static car25519(o: Float64Array): void {
    let v;
    let c = 1;
    for (let i = 0; i < 16; i++) {
      v = o[i] + c + 65535;
  ...
  static pack25519(o: Uint8Array, n: Float64Array): void {
    let b;
    const m = gf();
    const t = gf();
  ...
  static par25519(a: Float64Array): number {
    const d = new Uint8Array(32);
    this.pack25519(d, a);
    return d[0] & 1;
  }

  static unpack25519(o: Float64Array, n: Uint8Array): void {
    let i;
    for (i = 0; i < 16; i++) o[i] = n[2 * i] + (n[2 * i + 1] << 8);
    o[15] &= 0x7fff;
  }

  static a(o: Float64Array, a: Float64Array, b: Float64Array): void {
    for (let i = 0; i < 16; i++) o[i] = a[i] + b[i];
  }

  static z(o: Float64Array, a: Float64Array, b: Float64Array): void {
    for (let i = 0; i < 16; i++) o[i] = a[i] - b[i];
  }

  static m(o: Float64Array, a: Float64Array, b: Float64Array): void {
    let v;
    let c;
    let t0 = 0;
    let t1 = 0;
  ...
  static s(o: Float64Array, a: Float64Array): void {
    this.m(o, a, a);
  }

  static inv25519(o: Float64Array, i: Float64Array): void {
    const c = gf();
    let a;
  ...
  static pow2523(o: Float64Array, i: Float64Array): void {
    const c = gf();
    let a;
    for (a = 0; a < 16; a++) c[a] = i[a];
    for (a = 250; a >= 0; a--) {
  ...
  static cryptoHash(out: Uint8Array, m: Uint8Array, n: number): number {
  ...
  static scalarmult(p: Float64Array[], q: Float64Array[], s: Uint8Array): void {
    let b;
    let i;
    this.set25519(p[0], gf0);
    this.set25519(p[1], gf1);
    this.set25519(p[2], gf1);
    this.set25519(p[3], gf0);
    for (i = 255; i >= 0; --i) {
      b = (s[(i / 8) | 0] >> (i & 7)) & 1;
      this.cswap(p, q, b);
      this.add(q, p);
  ...
  static cryptoSign(sm: Uint8Array, m: Uint8Array, n: number, sk: Uint8Array): number {
    ...
    const pk = this.derivePublicFromSecret(sk);
  ...
  static unpackneg(r: Float64Array[], p: Uint8Array): -1 | 0 {
    const t = gf();
    const chk = gf();
    const num = gf();
    const den = gf();
  ...
  static cryptoSignOpen(m: Uint8Array, sm: Uint8Array, n: number, pk: Uint8Array): number {
    ...
    n -= 64;
    if (this.cryptoVerify32(sm, 0, t, 0)) {
      for (i = 0; i < n; i++) m[i] = 0;
      return -1;
    }
  ...
  static naclSign(msg: Uint8Array, secretKey: Uint8Array): Uint8Array {
  ...
  static signDetached(msg: Uint8Array, secretKey: Uint8Array): Uint8Array {
  ...
  static verifyDetached(msg: Uint8Array, sig: Uint8Array, publicKey: Uint8Array): boolean {
    ...
    return this.cryptoSignOpen(m, sm, sm.length, publicKey) >= 0;
  }
}
